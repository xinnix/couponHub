...
本项目已升级为真实的微信支付 V3 API，支持小程序内 JSAPI 支付、回调通知、退款等完整流程。
...
```
apps/api/certs/
├── apiclient_key.pem      # 商户 API 私钥（必须）
...
- `apiclient_key.pem` 是私钥文件，**绝对不能泄露**
...
| `WX_PAY_SERIAL_NO` | 证书序列号 | 商户平台 → API安全 → 证书详情 |
| `WX_PAY_PRIVATE_KEY_PATH` | 私钥文件路径 | 相对路径，默认 `./certs/apiclient_key.pem` |
...
- [ ] 商户号已获取
- [ ] API v3 密钥已设置
- [ ] 商户 API 证书已下载并放到 `apps/api/certs/`
- [ ] 证书序列号已配置
- [ ] 回调 URL 已配置且为 HTTPS
- [ ] 域名已在商户平台配置
- [ ] `.env` 文件中所有支付配置项已填写
